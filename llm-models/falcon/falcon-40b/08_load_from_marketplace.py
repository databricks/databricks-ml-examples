...
# MAGIC %md
# MAGIC # Loading `falcon-40b-instruct` model from Marketplace
# MAGIC
# MAGIC This example notebook demonstrates how to load the [falcon-40b-instruct](https://huggingface.co/tiiuae/falcon-40b-instruct) model from a Databricks Marketplace's Catalog ([see announcement blog](https://www.databricks.com/blog/llama-2-foundation-models-available-databricks-lakehouse-ai)).
# MAGIC
# MAGIC Environment:
...
catalog_name = "databricks_falcon-40b-instruct_models" # Default catalog name when installing the model from Databricks Marketplace
version = 1

# Create a Spark UDF to generate the response to a prompt
generate = mlflow.pyfunc.spark_udf(
    spark, f"models:/{catalog_name}.models.falcon-40b-instruct/{version}", "string"
)
