...
# MAGIC %md
# MAGIC # Fine tune `falcon-7b` model from marketplace with QLORA
# MAGIC
# MAGIC Databricks hosts the [falcon-7b](). This is a tutorial to show how to fine tune the models on the [databricks-dolly-15k](https://huggingface.co/datasets/databricks/databricks-dolly-15k) dataset.
# MAGIC
...
catalog_name = "databricks_falcon-7b" # Default catalog name when installing the model from Databricks Marketplace
version = 1

model_mlflow_path = f"models:/{catalog_name}.models.falcon-7b/{version}"

model_local_path = "/local_disk0/falcon-7b/"
model_output_local_path = "/local_disk0/falcon-7b-lora-fine-tune"
...
# MAGIC
# MAGIC In this section we will load the [falcon-7b](https://huggingface.co/tiiuae/falcon-7b) model installed from [Databricks marketplace]() saved in Unity Catalog to local disk, quantize it in 4bit and attach LoRA adapters on it.
...
registered_name = f"{catalog_name}.models.falcon-7b_fine_tune" # Note that the UC model name follows the pattern <catalog_name>.<schema_name>.<model_name>, corresponding to the catalog, schema, and registered model name
