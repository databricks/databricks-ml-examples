...
# MAGIC   - `g5.12xlarge` (4 A10 GPUs) on AWS: Note that the triton-based
# FlashAttention would not work for this model with smaller GPUs such as
# A10, set `USE_TRITON=False` below

# COMMAND ----------

# TODO: Please change this to False if not using A100-80GB GPU
import torch
...
# MAGIC ## Inference
# MAGIC
# MAGIC Note: This model requires that `trust_remote_code=True` be passed
# to the `from_pretrained` method. This is because MosaicML uses a custom
# model architecture that is not yet part of the transformers package.
