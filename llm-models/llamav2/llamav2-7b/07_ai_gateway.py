...
# The below assumes you've create an endpoint "llama2-7b-chat" according
# to 02_mlflow_logging_inference
...
# Databricks URL and token that would be used to route the Databricks
# serving endpoint
...
# MAGIC
# MAGIC Note that `mlflow.gateway.query` doesn't need to be run in the
# same notebook nor the same cluster, and it doesn't require the
# Databricks URL or API token to query it, which makes it convenient for
# multiple users within the same organization to access a served model.
