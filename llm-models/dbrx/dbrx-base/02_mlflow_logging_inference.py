...
# MAGIC %md
# MAGIC # Manage `dbrx-instruct` model with MLFlow on Databricks
# MAGIC
...
# MAGIC %pip install -U  torch==2.2.1  torchvision==0.17.2  transformers==4.38.2  accelerate==0.26.1  einops==0.7.0  flash-attn==2.5.2 
...
with mlflow.start_run() as run:
    mlflow.transformers.log_model(
        ...
        artifact_path="model",
        pip_requirements=["torch==2.2.1", "torchvision==0.17.2", "transformers==4.38.2", "accelerate==0.26.1", "einops==0.7.0", "flash-attn==2.5.2"],
        ...
        metadata={
            ...
            "databricks_model_size_parameters": "131596523520b"
        }
    )
...
registered_name = "models.default.dbrx-instruct"  # Note that the UC model name follows the pattern <catalog_name>.<schema_name>.<model_name>, corresponding to the catalog, schema, and registered model name
...
# MAGIC Once the model is registered, we can use API to create a Databricks GPU Model Serving Endpoint that serves the `dbrx-instruct` model.
# MAGIC
...
# MAGIC
# MAGIC Models in `dbrx-instruct` family are supported for Optimized LLM Serving, which provides an order of magnitude better throughput and latency improvement. 
