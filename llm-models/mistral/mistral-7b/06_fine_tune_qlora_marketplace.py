...
# MAGIC %md
# MAGIC # Fine tune Mistral-7B-v0.1 model from marketplace with QLORA
# MAGIC
# MAGIC Databricks hosts the Mistral-7B-v0.1 in [Databricks marketplace](). This is a tutorial to show how to fine tune the models on the [databricks-dolly-15k](https://huggingface.co/datasets/databricks/databricks-dolly-15k) dataset.
# MAGIC
...
model_mlflow_path = f"models:/{catalog_name}.models.mistral_7b_v0_1/{version}"

model_local_path = "/local_disk0/mistral_7b_v0_1/"
model_output_local_path = "/local_disk0/mistral_7b_v0_1_lora_fine_tune"
...
# MAGIC
# MAGIC In this section we will load the [Mistral-7B-v0.1](https://huggingface.co/mistralai/Mistral-7B-v0.1) model installed from [Databricks marketplace]((https://marketplace.databricks.com/details/46527194-66f5-4ea1-9619-d7ec6be6a2fa/Databricks_Llama-2-Models) saved in Unity Catalog to local disk, quantize it in 4bit and attach LoRA adapters on it.
...
class MISTRALQLORA(mlflow.pyfunc.PythonModel):
...
registered_name = f"{catalog_name}.models.mistral_7b_fine_tune" # Note that the UC model name follows the pattern <catalog_name>.<schema_name>.<model_name>, corresponding to the catalog, schema, and registered model name
